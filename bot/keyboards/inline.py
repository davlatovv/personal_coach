...
def reminder_confirm_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Сохранить", callback_data="reminder_confirm:yes")
    builder.button(text="❌ Отмена", callback_data="reminder_confirm:no")
    builder.adjust(2)
    return builder.as_markup()
...
def notification_action_keyboard(log_id: int) -> InlineKeyboardMarkup:
    ...
    builder.button(text="❌ Пропустить", callback_data=f"notif:skipped:{log_id}")
    builder.adjust(2)
    return builder.as_markup()
